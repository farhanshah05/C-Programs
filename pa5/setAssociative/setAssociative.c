...
void accessData (
    ...
    mem_addr_t reducedaddr = addr >> b; 
    mem_addr_t linesetter = reducedaddr & 0x03; 


    // Find the correctCacheSet cache set
    /* ... */
    cache_set_t correctCacheSet = cache[linesetter];

    // Cache hit
    /* ... */
    for (int i = 0; i < 4; i++){
        if (correctCacheSet[i].tag == tag && correctCacheSet[i].valid){
            correctCacheSet[i].recent_use = program_counter;
            (*hit_count)++;
            return;
        }
    }
    ...
    // Within the correctCacheSet cache set, find the least recently used line; that is where missed cache block will be placed
    /* ... */
    int numberLeastLines = 0;
    int recentCount = correctCacheSet[0].recent_use;

    for(int i = 1; i < 4; i++){
        if (correctCacheSet[i].recent_use < recentCount){
            recentCount = correctCacheSet[i].recent_use;
            numberLeastLines = i;
        }
    }
    ...
    /* ... */
    if (correctCacheSet[numberLeastLines].valid){
        correctCacheSet[numberLeastLines].recent_use = program_counter;
        correctCacheSet[numberLeastLines].tag = tag;
        (*eviction_count)++;
    }

    else {
        correctCacheSet[numberLeastLines].valid = 1;
        correctCacheSet[numberLeastLines].recent_use = program_counter;
        correctCacheSet[numberLeastLines].tag = tag;
    }
