...
int main(int argc, char* argv[])
{
    ...
    //CacheBlocking Algorithm
    for (size_t i = 0; i < n; i += variable) {
        for (size_t j = 0; j < n; j += variable) {
            for (size_t k = 0; k < n; k += variable) {
                for (size_t w = i; w < i + variable; w++) {
                    for (size_t z = k; z < k + variable; z++) {
                        for (size_t u = j; u < j + variable; u++) {
                            c[w * n + u] += a[w * n + z] * b [z * n + u];
                        }
                    }
                }
            }
