...
int main ( int argc, char* argv[] ) {
    ...
    size_t graphNodeCount = adjMatrixToList(filename, &adjacencyList);
    if (graphNodeCount == 0) {
    ...
    for (unsigned iter=0; iter<graphNodeCount-1; iter++) {
        ...
        double minWght = DBL_MAX; // If we find an edge with weight less than this minWght, and edge connects a new node to MST, then mark this as the minimum weight to beat.
        graphNode_t minSrc = -1;
        graphNode_t minDestination = -1;

        for (graphNode_t source=0; source<graphNodeCount; source++) {
            if (parents[source]!=-1) { // if already visited
                AdjacencyListNode* nbor = adjacencyList[source].next;
                while(nbor){
                    graphNode_t currNode = nbor->graphNode;
                    if(nbor->weight<minWght && parents[currNode] == -1){
                        minDestination = nbor->graphNode;
                        minSrc = source;
                        minWght = nbor->weight;
                    }
                    nbor = nbor->next;
        ...
        parents[minDestination]=minSrc; // we found the minimum weight
    }

    // Using the fully populated parents array, print the edges in the MST.
    /* ... */
    for(int pint = 0; pint<graphNodeCount; pint++){
        if(parents[pint]!=pint){
            printf("%ld %d\n", parents[pint], pint);
        }
    }
